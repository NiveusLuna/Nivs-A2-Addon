...
float KEYBOARD_SCROLL_RATE =  2.0;
...
float INITIAL_SCROLL_SPEED = 1;			    // Initial speed
int cfgTACTICAL_WHERE_MODE = 3;				// use last selected end path or what you get from shift
...
float cfgINITIAL_STARDATE = 56123.1;	// stardate at mission start if not specified, orig: 56123.1
float cfgSTARDATE_PER_SECOND = 0.01389;		// for every "game second (more or less real time second), this many stardate intervals pass
...
float EASY_MINING_MODIFIER = 2.0;
float HARD_MINING_MODIFIER = 0.5;

// AI special weapon fire rate
float EASY_SPECIAL_WEAPON_FIRE_MODIFIER = 20.0;
float DEFAULT_SPECIAL_WEAPON_FIRE_MODIFIER = 4.0;
float HARD_SPECIAL_WEAPON_FIRE_MODIFIER = 0.05;

// Build cost multiplier handicap for hard AI
float HARD_AI_COST_HANDICAP = 0.6;
float HARD_AI_SPEED_HANDICAP = 0.7;
...
float cfg8472_RESOURCE_DISTANCE = 300.0;

// what is too close to the resource?
float cfg8472_RESOURCE_INSIDE_DISTANCE = 200.0;
...
float cfg_MIN_MOVE_WARP_DISTANCE = 1100.0;
...
float cfg_WARP_INHIBITION_BUFFER = 80.0;
...
int cfg_ALMOST_INTRAVERSIBLE = 300;
...
float cfgPLANET_DEWARP_RADIUS_MULTIPLIER = 5;
// Radius multiplier for black holes, since hey, they got gravity
float cfgBLACKHOLE_DEWARP_RADIUS_MULTIPLIER = 125.0;
...
int STARTING_DILITHIUM =		3500;
int STARTING_CREW =			750;
int STARTING_CREW_CAPACITY =		250;
int STARTING_LATINUM =			1000;
int STARTING_METAL =			12500;
...
float cfgFADE_OUT_MIN_FOV = 0.5;
// ratio of radius/distance at which fade out is maximum
float cfgFADE_OUT_MAX_FOV = 0.8;
...
float REPAIR_SHIP_DISTANCE = 175;
